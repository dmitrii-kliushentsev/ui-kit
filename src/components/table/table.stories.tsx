...
storiesOf('Table', module).add('Table', () => {
  ...
  return (
    <Table data={data} sort={sort} onSort={(cellSort) => setSort(cellSort)}>
      <Column
        name="lambdaFunction"
        label="Lambda Function"
        Cell={({ value, item: { arrowFunction } }) => (
          <OverflowText>{value || arrowFunction}</OverflowText>
        )}
      />
      <Column
        name="icon"
      ...
      />
      <Column name="funcExpression" HeaderCell={() => <div>Func Expression</div>} />
      <Column
        name="anonymousFuncExpression"
        HeaderCell={() => <pre>Anonymous Func Expression</pre>}
      />
    </Table>
  );
});
