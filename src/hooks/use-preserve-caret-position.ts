...
export const usePreserveCaretPosition = (normalize: (str: string) => string) => {
  ...
  const parse = (value: string, caretPosition: number) => {
    ...
    const parseBeforeCaret = normalize(beforeCaret);
    const parseAfterCaret = normalize(afterCaret);
